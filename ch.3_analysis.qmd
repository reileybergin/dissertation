...
calculate_sem_anova <- function(data, Zcrit = 1.96) {
...
calculate_bayes_sem <- function(models_and_metadata_list) {
  ...
  process_single_model <- function(model_and_metadata) {
    ...
    prob_good_cov <- (mean(stats_distribution$cov < 10)) * 100
    prob_questionable_cov <- mean((stats_distribution$cov >= 10 & stats_distribution$cov <= 15)) *100
    prob_poor_cov <- (mean(stats_distribution$cov > 15)) * 100
    
    # Calculate quantiles for the new distribution
    ...
    result_row <- data.frame(variable = model_and_metadata$variable,
                             ...
                             quantile_66_sem_sqrt2 = quantile_66_sem_sqrt2,
                             quantile_90_sem_sqrt2 = quantile_90_sem_sqrt2,
                             quantile_99_sem_sqrt2 = quantile_99_sem_sqrt2,
                             ...
                             prob_good_cov = prob_good_cov,
                             prob_questionable_cov = prob_questionable_cov,
                             prob_poor_cov = prob_poor_cov)
...
# Running ---
run_pta_mean <- c(8.0, 2.0)
run_pta_chg <- c(0.0, 0.50) 

# Walking ---
walk_pta_mean <- c(5.0, 1.0)
walk_pta_chg <- c(0.0, 0.50)
```
...
ggplot(sim_data, aes(x = time2_value)) +
  geom_dotplot(binwidth = 0.2, dotsize = 0.2, fill = "#1E90FF") +
  scale_y_continuous(NULL, breaks = NULL) +
  scale_x_continuous(name = "Mean Time 2", limits = c(0, NA)) +
  scale_color_brewer() 
```
...
# Low Back Peak Accel
bm_run_lb_pa <- fit_bayes_model(reliability_data, "lb_res_pk_accel_g", "run", c(4.0, 1.0), c(0.0, 0.050))

# RMS values---

# Low Back RMS x
bm_run_lb_rms_x <- fit_bayes_model(reliability_data, "lb_accel_x_rms_g", "run", c(0.50, 0.10), c(0.0, 0.050))
# Low Back RMS Y
bm_run_lb_rms_y <- fit_bayes_model(reliability_data, "lb_accel_y_rms_g", "run", c(1.2, 0.12), c(0.0, 0.050))
# Low Back RMS z
bm_run_lb_rms_z <- fit_bayes_model(reliability_data, "lb_accel_z_rms_g", "run", c(0.50, 0.10), c(0.0, 0.050))
```
...
launch_shinystan(bm_run_avg_pta$model)
```

```{r}
# Model summaeries
summary(bm_run_avg_pta$model)
```

```{r}
# View intercepts for each subject
coef(bm_run_avg_pta$model)
```

```{r}
# Intercepts of each subject
bm_run_avg_pta$model %>%
  spread_draws(b_Intercept, r_sub_id[sub_id,]) %>%
  mutate(sub_id_mean = b_Intercept + r_sub_id) %>%
  ggplot(aes(y = sub_id, x = sub_id_mean, fill = after_stat(abs(x) > 12 | abs(x) < 8))) +
  stat_halfeye() +
  geom_vline(xintercept = c(8.0, 12.0), linetype = "dashed") +
...
dfs[["rt_lt_avg_res_pk_accel_g_run"]] %>%
  data_grid(sub_id, time_pt) %>%
  add_predicted_draws(bm_run_avg_pta$model) %>%
...
dfs[["rt_lt_avg_res_pk_accel_g_run"]] %>%
  data_grid(sub_id, time_pt) %>%
  add_epred_draws(bm_run_avg_pta$mode) %>%
  ggplot(aes(x = .epred, y = sub_id)) +
...
dfs[["rt_lt_avg_res_pk_accel_g_run"]] %>%
  data_grid(sub_id, time_pt) %>%
  add_epred_draws(bm_run_avg_pta$model, dpar = c("mu", "sigma")) %>%
  sample_draws(30) %>%
  ggplot(aes(y = sub_id)) +
  stat_slab(aes(xdist = dist_normal(mu, sigma)), 
    slab_color = "gray65", alpha = 1/10, fill = NA
  ) +
  geom_point(aes(x = value), data = dfs[["rt_lt_avg_res_pk_accel_g_run"]], shape = 21, fill = "#9ECAE1", size = 2)
```
