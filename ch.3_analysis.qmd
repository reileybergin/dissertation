...
fit_bayes_model <- function(data, variable_filter, run_type_filter, intercept_prior, b_prior) {
  ...
  # Priors Only Model for prior predictive checks
  ...
  return(list(model = bayes_model, priors_model =  bayes_priors_only_model, 
...
calculate_bayes_sem <- function(models_and_metadata_list) {
  ...
  process_single_model <- function(model_and_metadata) {
    ...
    prob_less_10_cov <- (mean(stats_distribution$cov < 10)) * 100
    prob_10_to_15_cov <- mean((stats_distribution$cov >= 10 & stats_distribution$cov <= 15)) *100
    ...
    result_row <- data.frame(variable = model_and_metadata$variable,
                             ...
                             quantile_95_sem_sqrt2 = quantile_95_sem_sqrt2,
                             ...
                             prob_less_10_cov = prob_less_10_cov,
                             prob_10_to_15_cov = prob_10_to_15_cov,
                             prob_greater_15_cov = prob_greater_15_cov)
...
dfs[["rt_lt_avg_res_pk_accel_g_walk"]] %>%
...
# Peak Tibal Acceleration (PTA) of Resultant
run_pta_mean <- c(9.1, 3.13)
run_pta_chg <- c(0.13, 0.50) 

# Low Back Peak Acceleration (LB-PA) of Resultant
run_lb_pa_mean <- c(3.77, 0.81)
run_lb_pa_chg <- c(0.04, 0.50) 
...
intercept_priors <- walk_pta_mean
beta_priors <- walk_pta_chg
...
```{r}
model<-bm_walk_avg_pta$model
priors_model <- bm_walk_avg_pta$priors_model
table <- dfs[["rt_lt_avg_res_pk_accel_g_walk"]]
```
...
bayes_sem_tbl <- calculate_bayes_sem(list(
  bm_run_avg_pta, bm_run_rt_pta, bm_run_lt_pta, 
  bm_run_lb_pa, bm_run_lb_rms_x, bm_run_lb_rms_y, bm_run_lb_rms_z, bm_walk_avg_pta))
```
