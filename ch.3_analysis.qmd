...
```{r}
results_freq <- correlation(run_tibia_force_mag)
results_freq
```
...
anova_model <- aov(value ~ time + Error(sub_id/time), data = run_rt_leg_pta_long)
...
grand_mean <- mean(run_rt_leg_pta_long$value)
...
# Your existing code to calculate cov_distribution and cov_89_credible_interval...

# Create a density plot of the CoV distribution
p <- ggplot(cov_distribution, aes(x=cov)) +
  geom_density(fill="blue", alpha=0.5) +
  geom_vline(xintercept=cov_89_credible_interval[1], color="red", linetype="dashed") +
  geom_vline(xintercept=cov_89_credible_interval[2], color="red", linetype="dashed") +
  labs(title="Density Plot of Coefficient of Variation (CoV)",
       subtitle=paste0("89% Credible Interval: [", round(cov_89_credible_interval[1], 2), ", ", round(cov_89_credible_interval[2], 2), "]"),
...
# You can print or use the estimated_mode as you need
print(estimated_mode)

# Optionally, add a vertical line for the estimated mode to
# Optionally, add a vertical line for the estimated mode to your plot
p + geom_vline(xintercept=estimated_mode, color="green", linetype="dotted") +
  geom_text(aes(x=estimated_mode, label=paste("Estimated Mode:", round(estimated_mode, 2)), y=max(dens$y)), vjust=-0.5, color="green")
```
