...
full_dataset <- read_csv("data/ch.4_data.csv") %>%
  ...
  filter(sub_id != "Sub007", sub_id != "Sub008") %>%
...
# Calculate average for left and right tibia ---
...
### Shock attenuation in the time domain

$$
\begin{align*}
\text{Shock Attenuation} & = \left( 1 - \frac{\text{Low Back Peak Accel}}{\text{Tibia Peak Accel}} \right) \times 100 \\\end{align*}
$$ {#eq-shock-attenutation}

```{r}
#| label: Calc Shock Attenuation

# Join tables to calculate across columns
shock_attenuation <- full_dataset %>%
  filter(variable == "lb_res_pk_accel_g") %>%
  left_join(full_dataset %>% filter(variable == "rt_lt_avg_res_pk_accel_g"),
            by = c("sub_id", "run_type")) %>%
  mutate(shock_atten = (1 - (value.x / value.y)) * 100) %>%
  # Select relevant columns and rename for clarity
  select(sub_id, run_type, variable = variable.x, shock_atten)
