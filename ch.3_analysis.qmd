...
# Calculates the mode based on the density of the ICC values. 
# NOTE: This is a simple approach and may not be perfect for multimodal distributions.
get_mode <- function(x) {
  dens <- density(x)
  x[dens$y == max(dens$y)]
}
...
calculate_bayes_icc <- function(model_and_metadata) {
...
```{r}
bayes_icc_tbl <- calculate_bayes_icc(run_avg_pta)
```
...
```{r}
run_rt_pta_2 <- fit_bayes_model(reliability_data, "rt_res_pk_accel_g", "run", c(8, 3), c(0, 0.05))
```

```{r}
run_rt_pta_3 <- fit_bayes_model(reliability_data, "rt_res_pk_accel_g", "run", c(8, 3), c(0.1, 0.05))
```
...
# ICC point estimate
icc_bayes <- icc(run_avg_pta$model)
icc_bayes$ICC_adjusted
```
...
# Extracting posterior samples
posterior_samples <- as_draws_df(run_avg_pta$model, variable = c("sd_sub_id__Intercept", "sigma"))
