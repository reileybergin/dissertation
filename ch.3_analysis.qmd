...
intercept_mean <- run_lb_pa_mean[1]
intercept_sd <- run_lb_pa_mean[2]
beta_mean <- run_lb_pa_chg[1]
beta_sd <- run_lb_pa_chg[2]
...
```{r}
model<- bm_run_lb_pa$model
table <- dfs[["rt_lt_avg_res_pk_accel_g_run"]]
```
...
table%>%
  ...
  geom_point(aes(x = value), data = dfs[["lb_res_pk_accel_g_run"]], shape = 21, fill = "#9ECAE1", size = 2)
```
